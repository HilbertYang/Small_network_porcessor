...
module tb_cpu_gpu_dmem_top;
    ...
    // SUB_I16  RD[4xi16] = RS1[4xi16] - RS2[4xi16]
    function [31:0] gpu_sub_i16;
        input [3:0] rd;
        input [3:0] rs1;
        input [3:0] rs2;
        begin gpu_sub_i16 = {5'h02, rd, rs1, rs2, 15'h0}; end
    endfunction

    // MAX_I16  RD[4xi16] = max(RS1[4xi16], RS2[4xi16])
    function [31:0] gpu_max_i16;
        input [3:0] rd;
        input [3:0] rs1;
        input [3:0] rs2;
        begin gpu_max_i16 = {5'h03, rd, rs1, rs2, 15'h0}; end
    endfunction

    // SHIFTLV  RD = RS1 <<< imm15  (currently acts as RD = RS1)
    function [31:0] gpu_shiftlv;
        input [3:0] rd;
        input [3:0] rs1;
        input [14:0] imm15;
        begin gpu_shiftlv = {5'h07, rd, rs1, 4'h0, imm15}; end
    endfunction

    // SHIFTRV  RD = RS1 >>> imm15  (currently acts as RD = RS1)
    function [31:0] gpu_shiftrv;
        input [3:0] rd;
        input [3:0] rs1;
        input [14:0] imm15;
        begin gpu_shiftrv = {5'h08, rd, rs1, 4'h0, imm15}; end
    endfunction

    // MAC_BF16  RD[4xbf16] = RS1 * RS2 + RD  (RD is also accumulator)
    function [31:0] gpu_mac_bf16;
        input [3:0] rd;
        input [3:0] rs1;
        input [3:0] rs2;
        begin gpu_mac_bf16 = {5'h09, rd, rs1, rs2, 15'h0}; end
    endfunction

    // MUL_BF16  RD[4xbf16] = RS1 * RS2
    function [31:0] gpu_mul_bf16;
        input [3:0] rd;
        input [3:0] rs1;
        input [3:0] rs2;
        begin gpu_mul_bf16 = {5'h0a, rd, rs1, rs2, 15'h0}; end
    endfunction
    ...
    initial begin
        ...
        imem_write(9'd3,    gpu_ld_param(4'd4, 3'd4));            // R4 = param[4] limit
        imem_write(9'd4,    gpu_mov(4'd5, 15'd0));                // R5 = Counter = 0

        imem_write(9'd5,    gpu_nop(1'b0));                      
        imem_write(9'd6,    gpu_nop(1'b0));

        imem_write(9'd7,    gpu_setp_ge(4'd5, 4'd4));             // PRED = (R5 >= R4)
        imem_write(9'd8,    gpu_bpr(9'd19));                      // if PRED: goto 19 (RET)
        imem_write(9'd9 ,   gpu_ld64 (4'd10, 4'd1, 15'd0));       // R10 = DMEM[R1]  A
        imem_write(9'd10,   gpu_ld64 (4'd11, 4'd2, 15'd0));       // R11 = DMEM[R2]  B
        imem_write(9'd11,   gpu_ld64 (4'd12, 4'd3, 15'd0));       // R12 = DMEM[R3]  C (accumulator)
        imem_write(9'd12,   gpu_addi64(4'd1, 4'd1, 15'd1));       // R1++
        imem_write(9'd13,   gpu_addi64(4'd2, 4'd2, 15'd1));       // R2++
        imem_write(9'd14,   gpu_mac_bf16(4'd12, 4'd10, 4'd11));   // R12 = R10*R11 + R12
        imem_write(9'd15,   gpu_br(9'd7));                        // goto 7  (delay slots: 16, 17)
        imem_write(9'd16,   gpu_addi64(4'd5, 4'd5, 15'd4));       // [delay slot] R5 += 4
        imem_write(9'd17,   gpu_st64 (4'd12, 4'd3, 15'd0));       // [delay slot] DMEM[R3] = R12
        imem_write(9'd18,   gpu_addi64(4'd3, 4'd3, 15'd1));       // R3++

        imem_write(9'd19,   gpu_ret(1'b0));                       // RET  (BPR exit target)
        imem_write(9'd20,   gpu_nop(1'b0));
        ...
        imem_write(9'd7,  cpu_mov(4'd6, 8'd11));        // MOV R6, #11
        imem_write(9'd8,  cpu_wrp(4'd6, 3'd4));         // WRP R5, #4  param[4]=11
        imem_write(9'd9,  cpu_nop(1'b0));
        imem_write(9'd10,  cpu_gpurun(1'b0));            // GPURUN
        imem_write(9'd11,  cpu_nop(1'b0));
        imem_write(9'd12, cpu_nop(1'b0));
        imem_write(9'd13, cpu_b(24'hFFFFFE));           // B -2  (loop forever)
        ...
        dmem_read_check(8'd20, 64'h4120_40A0_4000_3F80, 1);
        dmem_read_check(8'd21, 64'h4248_4214_41D0_4188, 2);
        dmem_read_check(8'd22, 64'h42F4_42CA_42A4_4282, 3);
        dmem_read_check(8'd23, 64'h0000_0000_0000_0000, 4);
        dmem_read_check(8'd24, 64'h0000_0000_0000_0000, 5);
        dmem_read_check(8'd25, 64'h0000_0000_0000_0000, 6);
